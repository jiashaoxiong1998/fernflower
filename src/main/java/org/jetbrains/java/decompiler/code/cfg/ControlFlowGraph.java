...
public class ControlFlowGraph implements CodeConstants {
  ...
  private void setSubroutineEdges() {
    ...
    for (BasicBlock block : blocks) {
      ...
      if (block.getSeq().getLastInstr().opcode == opc_jsr) {
  ...
  private Set<BasicBlock> getJsrRange(BasicBlock jsr, BasicBlock ret) {
    ...
    while (!lstNodes.isEmpty()) {
      ...
      for (int j = 0; j < 2; j++) {
        List<BasicBlock> lst;
        if (j == 0) {
          if (node.getLastInstruction().opcode == opc_ret) {
      ...
      for (int j = 0; j < 2; j++) {
        ...
        for (int i = lst.size() - 1; i >= 0; i--) {
          ...
          if (mapNewNodes.containsKey(childid)) {
            ...
            copy.id = ++last_id;
            // copy all successors
            if (copy.getLastInstruction().opcode == opc_ret &&
  ...
  private static void removeJsrInstructions(ConstantPool pool, BasicBlock block, DataPoint data) {
    ...
    for (int i = 0; i < seq.length(); i++) {
      ...
      VarType var = null;
      if (instr.opcode == opc_astore || instr.opcode == opc_pop) {
    ...
    for (int i = 0; i < block.getSuccExceptions().size(); i++) {
      ...
      if (suc.mark != 1) {
        ...
        point.getStack().push(new VarType(TYPE_OBJECT, 0, null));
